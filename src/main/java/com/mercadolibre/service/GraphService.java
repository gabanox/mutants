	...
	public int[][] generateAdjacencyMatrix(String[][] g){
		...
		for(int row = 0; row < A.length; row++){
			for(int column = 0; column < A.length; column++){
				...
				if(row <= A.length && column > 0){
					if(g[row][column] == g[row][(column-1)] && MutantHelper.isMutantNitrogenBase(g[row][column])){
						A[row][column] = 1;
						A[row][(column-1)] = 1;
					}
				}
		...
		for(int column = 0; column < A.length; column++){
			for(int row = 0; row < A.length; row++){
				...
				if(column <= A.length && row > 0){
					if(row > 0){
						if(g[row][column] == g[(row-1)][column] && MutantHelper.isMutantNitrogenBase(g[row][column])){
							A[row][column] = 1;
							A[(row-1)][column] = 1;
						}
					}
		...
		for(int i = 0; i < A.length; i++){
			for(int j = 0; j < A.length; j++){
				...
				if(i <= A.length && j > 0){
					if(i == j){
						if(g[j][i] == g[(j-1)][(i-1)] && MutantHelper.isMutantNitrogenBase(g[j][i])){
							A[j][i] = 1;
							A[(j-1)][(i-1)] = 1;
						}
					}
